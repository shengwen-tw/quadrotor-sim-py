...
import care_sda  # 需要你前面那個 care_sda.py
from se3_math import SE3

try:
    from scipy.linalg import solve_continuous_are as scipy_care
except Exception:
    scipy_care = None
...
class LQRController:
    """
    LQR tracker that (re)linearizes about the current state at each step,
    solves CARE via SDA (with SciPy fallback), and applies u = u_ff - K(x - x0).
    State ordering (12x1) matches the MATLAB example:

        [ roll; pitch; yaw;
          p; q; r;            (body angular rates)
          u; v; w;            (body-frame velocities)
          x; y; z ]           (world/NED position)

    Inputs:
        env.uav_dynamics must provide:
            mass, J(3x3), g, dt,
            x(3), v(3), R(3x3), W(3),
        and settable:
            f (3), M(3)
    ...
    def _solve_care(self, A, B, H, R):
        # Prefer your SDA implementation; fall back to SciPy if available
        t0 = time.perf_counter()
        try:
            X = care_sda.care_sda(A, B, H, R)
        except Exception:
            if scipy_care is None:
                raise
            X = scipy_care(A, B, H, R)
    ...
    def run(self, env):
        ...
        env.uav_dynamics.f = uav_ctrl_f
        env.uav_dynamics.M = uav_ctrl_M
        ...
        self.ex_arr[:, self.idx] = env.uav_dynamics.x - xd
        self.ev_arr[:, self.idx] = env.uav_dynamics.v - vd
